...
// The fetching logic is moved to the parent (IALResources).
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 w-full">
      {subjects.map((name, index) => {
        ...
        return (
          <Link key={index} href={`/subjects/${slug}?choice=option1`}>
...
export default function IALResources() {
  ...
  // Fetch subjects for IAL qualification
  useEffect(() => {
    async function fetchSubjectsData() {
      ...
      const { data, error: fetchError } = await supabase
        ...
        .eq('syllabus_type', 'IAL'); // Filter by IAL syllabus type

      if (fetchError) {
        console.error("Error fetching IAL subjects:", fetchError.message);
        setError(fetchError);
      } else if (data) {
  ...
  if (error) {
    return (
      <main className="min-h-screen bg-white flex items-center justify-center">
        <p className="text-xl text-red-600">Error loading IAL subjects: {error.message}</p>
      </main>
    );
  }
        ...
        <div className="w-full max-w-5xl px-4">
          ...
          <div className="inline-flex items-center justify-center px-4 py-2 mb-8 rounded-md" style={{ border: "1.5px solid #DBDBDB", fontFamily: "Poppins, sans-serif" }}>
            <span className="text-md font-medium text-black tracking-[-0px]">
              <span className="font-[501]">Qualification:</span> IAL
            </span>
          </div>
          ...
          <h3 className="text-sm sm:text-md lg:text-lg font-[500] leading-6 text-[#707070] mb-8 text-left max-w-4xl tracking-[-0.020em]" style={{ fontFamily: "Poppins, sans-serif" }}>
            Access a wide range of Edexcel IAL resources—all in one place. Whether you're brushing up on concepts or aiming to master exam strategies, these materials are designed to support your revision and boost your performance
          </h3>

          {/* This component will now receive and display the IAL subject buttons */}
          {subjects.length > 0 ? (

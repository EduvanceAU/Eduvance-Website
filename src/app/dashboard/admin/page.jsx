        ...
        {!staffUser ? (
          ...
          <>
            ...
            <div className="flex space-x-4 border-b mb-4">
              ...
              <button onClick={() => setActiveTab('leaderboard')} className={`py-2 px-4 flex items-center gap-1 rounded-t-lg ${activeTab === 'leaderboard' ? 'border-b-2 border-blue-600 font-semibold text-blue-700 bg-blue-50' : 'text-gray-500 hover:text-blue-600'}`}><svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M3 17v-2a4 4 0 014-4h10a4 4 0 014 4v2" /><circle cx="12" cy="7" r="4" /></svg>Leaderboard</button>
            </div>
            {message && (
                ...
                <form onSubmit={handleAddSubject} className="space-y-4 bg-blue-50 p-4 rounded-2xl border border-blue-100">
                  ...
                  <div className="bg-white border border-blue-200 rounded-lg p-2 mt-2">
                    ...
                    <div className="flex flex-row gap-3 mb-2 items-center">
                      ...
                      <button type="button" onClick={handleAddUnit} className="bg-blue-500 hover:bg-blue-600 text-white px-4 py-2 rounded text-sm whitespace-nowrap">Add</button>
                    </div>
                    {newUnits.length > 0 && (
                      <ul className="list-disc pl-5 mt-2 space-y-1">
                        {newUnits.map((unit, idx) => (
                          <li key={idx} className="flex items-center gap-2 text-sm mb-1">
                            ...
                            <button type="button" onClick={() => handleRemoveUnit(idx)} className="ml-2 text-red-500 hover:text-red-700 text-xs">Remove</button>
                          </li>
                        ))}
                      </ul>
                    )}
            ...
            {activeTab === 'leaderboard' && (
              <>
                <h2 className="text-xl font-semibold text-blue-700 mb-2 flex items-center gap-2"><svg className="w-5 h-5 text-blue-500" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" d="M3 17v-2a4 4 0 014-4h10a4 4 0 014 4v2" /><circle cx="12" cy="7" r="4" /></svg>Leaderboard</h2>
                {loadingLeaderboard ? (
                ...
                <div className="bg-white rounded-2xl shadow-lg p-6 w-full max-w-lg relative">
                  ...
                  <div className="space-y-3">
                    ...
                    {(() => {
                      ...
                      if (Array.isArray(units) && units.length > 0) {
                        return (
                          <select name="unit_chapter_name" value={editForm.unit_chapter_name} onChange={handleEditFormChange} className="w-full border p-2 rounded-md focus:ring-2 focus:ring-blue-300 focus:border-blue-500 transition">
                            <option value="">Select Unit/Chapter (optional)</option>
                            {units.map((unit, idx) => (
                              <option key={unit.code || unit.name || idx} value={unit.unit || unit.name}>{unit.unit ? `${unit.unit} - ${unit.name}` : unit.name}</option>
                            ))}

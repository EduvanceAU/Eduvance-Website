...
async function data() {
  ...
  try {
    ...
    for (let subject of subject_list) {
        const one_timer = {url:`${base_url}/subjects/${toKebabCase(subject.name).toLowerCase()}`, lastModified: date.toISOString().split('T')[0], changeFrequency: 'monthly', priority: 0.9}
        if (!urls.some(entry => entry.url === one_timer.url)){
            urls.push(one_timer)
        }
      
        if(subject.syllabus_type === "IAL"){
            urls.push({url:`${base_url}/subjects/${toKebabCase(subject.name).toLowerCase()}/IAL/communityNotes`, lastModified: date.toISOString().split('T')[0], changeFrequency: 'weekly', priority: 0.8});
            urls.push({url:`${base_url}/subjects/${toKebabCase(subject.name).toLowerCase()}/IAL/resources`, lastModified: date.toISOString().split('T')[0], changeFrequency: 'weekly', priority: 0.8});
            urls.push({url:`${base_url}/subjects/${toKebabCase(subject.name).toLowerCase()}/IAL/pastpapers`, lastModified: date.toISOString().split('T')[0], changeFrequency: 'weekly', priority: 0.8});
        }
        else if(subject.syllabus_type === "IGCSE"){
            urls.push({url:`${base_url}/subjects/${toKebabCase(subject.name).toLowerCase()}/IGCSE/communityNotes`, lastModified: date.toISOString().split('T')[0], changeFrequency: 'weekly', priority: 0.8});
            urls.push({url:`${base_url}/subjects/${toKebabCase(subject.name).toLowerCase()}/IGCSE/resources`, lastModified: date.toISOString().split('T')[0], changeFrequency: 'weekly', priority: 0.8});
            urls.push({url:`${base_url}/subjects/${toKebabCase(subject.name).toLowerCase()}/IGCSE/pastpapers`, lastModified: date.toISOString().split('T')[0], changeFrequency: 'weekly', priority: 0.8});
        }
    }
    return urls

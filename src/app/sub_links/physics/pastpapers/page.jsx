...
export default function PastPapersPage() {
  ...
  return (
    <main className="min-h-screen bg-white flex flex-col items-start justify-start py-10 px-4 sm:px-6 md:px-12 lg:px-20">
      <h1 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-[#153064] mb-8 w-full max-w-6xl text-left tracking-[-0.035em]" style={{ fontFamily: 'Poppins, sans-serif' }}>
          ...
          <div key={year}>
            ...
            {sessions.map((session) => (
              <div key={`${year}-${session.label}`} className="mb-4">
                ...
                <div className="w-full bg-blue-100 rounded-t grid grid-cols-2 px-4 py-2 mb-1 text-[#153064]" style={{ fontFamily: 'Poppins, sans-serif' }}>
                  <span className="font-semibold text-sm text-left">Exam</span>
                  <span className="font-semibold text-sm text-left">Marking Scheme</span>
                </div>

                <div className="grid grid-cols-1 sm:grid-cols-[minmax(0,_1fr)_minmax(0,_1fr)] gap-x-4 sm:gap-x-10 gap-y-2">
                  {units.map((unit) => (
                    <>
                      <Link

...
const subjects = [
  { name: "Physics", link: "/subjects/physics/IGCSE/pastpapers" },
  { name: "Chemistry", link: "/subjects/chemistry/IGCSE/pastpapers" },
  { name: "Biology", link: "/subjects/biology/IGCSE/pastpapers" },
  { name: "Maths", link: "/subjects/maths/IGCSE/pastpapers" },
];
...
export default function IGCSEPastPapersPage() {
  ...
  const filteredPapers = papers && papers.filter(paper => {
    ...
    // Unit filter
    if (units.length > 0) {
      const selectedUnitCodes = units.map(unit => unit.unit);
                ...
                <div className="absolute z-10 bg-white shadow-lg rounded-lg mt-2 py-2 w-max max-h-60 overflow-y-auto border border-gray-200">
                  {units.map((unit, index) => (
                    <div
                      ...
                      className={`cursor-pointer px-4 py-2 text-sm flex items-center transition-colors
                        ${expandedUnits[unit.unit] ? 'bg-blue-50 text-blue-700' : 'hover:bg-gray-100 text-gray-900'}`}
                      style={{ fontFamily: "Poppins, sans-serif" }}
                    >
                      {/* Square Checkbox */}
                      <input
                        type="checkbox"
                        checked={expandedUnits[unit.unit]}
                        onChange={() => {}} // onChange is required but we handle click on div
                        className="form-checkbox h-4 w-4 text-blue-600 rounded mr-2"
                      />
                    ...
                    >
                      ...
                      <div className="p-6"> {/* Padding for the content inside the card */}
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6"> {/* Grid for Unit Cards */}
                          ...
                          {units
                            .map((unit) => {
                              const paperData = groupedPapers?.[year]?.[session.value]?.[unit.unit];

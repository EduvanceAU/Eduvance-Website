  ...
  useEffect(() => {
    ...
    const fetchUnits = async () => {
      ...
      if (subjectError || !subjectData) {
        setError(subjectError || new Error(`Subject "chemistry" not found for units.`));
        return;
      }
  ...
  useEffect(() => {
    ...
    async function fetchNotes() {
      ...
      const { data: subjectData, error: subjectError } = await supabase
        ...
        .single();
      if (subjectError || !subjectData) {
        setError(subjectError || new Error(`Subject "chemistry" not found for notes.`));
        setLoading(false);
        return;
      }
          ...
          <div className="blur-sm pointer-events-none select-none">
            ...
            >
              <span className="text-md font-medium text-black tracking-tight">
                <span className="font-[501]">Exam code:</span> WCH11/XCH11/YCH11 {/* This now uses the state */}
              </span>
            </div>

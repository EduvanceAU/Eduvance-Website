...
export default function IGCSEPastPapersPage() {
  ...
  const fetchPapers = async () => {
    ...
    try {
      ...
      if (subjectError || !subjectData) {
        const errMsg = subjectError ? subjectError.message : `Subject "economics" not found.`;
      ...
      console.debug('fetchPapers: Subject ID found:', subjectId);
  ...
  const fetchUnits = async () => {
    ...
    try {
      ...
      if (subjectError || !subjectData) {
        const errMsg = subjectError ? subjectError.message : `Subject "economics" not found for units.`;
        setError(new Error(errMsg));
  ...
  // Initial data fetch for papers and units
  useEffect(() => {
    fetchPapers();
    fetchUnits();
  }, []); // Run once on mount
  ...
  useEffect(() => {
    const checkAndRefetchPastPapers = () => {
      ...
      if (pastPaperLinksContainerRef.current) {
        // Look for any 'a' tags within the specified container that are children of a div with class 'space-y-2'
        // This targets the specific links for question paper, mark scheme, and examiner report.
        ...
        // If no links are found AND we have no papers in state (meaning the fetch might have failed or returned empty)
    ...
    // Set a timeout to check after the initial render and data processing
    ...
    <>
      <main className="min-h-screen bg-white flex flex-col items-center justify-start py-10 m-10">
        <div className="w-full max-w-5xl px-4">
          ...
          >
            IGCSE <span className="bg-[#1A69FA] px-2 py-1 -rotate-1 inline-block"><span className="text-[#FFFFFF]">Economics</span></span> Past Papers
          </h1>
          ...
          >
            <span className="text-md font-medium text-black tracking-tight">
              <span className="font-[501]">Exam code:</span> ECO
            </span>
          </div>
          ...
          <h3
            ...
            style={{ fontFamily: "Poppins, sans-serif" }}
          >
            Explore our collection of Edexcel IGCSE Economics Past Papers and Mark Schemes below. Practicing with IGCSE Economics past papers is one of the most effective ways to pinpoint the topics that need more focus—helping you revise smarter and prepare confidently for your upcoming exam
          </h3>

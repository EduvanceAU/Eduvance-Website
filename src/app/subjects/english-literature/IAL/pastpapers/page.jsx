...
export default function IALPastPapersPage() {
  ...
  const fetchPapers = async () => {
    ...
    try {
      ...
      if (subjectError || !subjectData) {
        const errMsg = subjectError ? subjectError.message : `Subject "english-literature" not found.`;
  ...
  const fetchUnits = async () => {
    ...
    try {
      ...
      if (subjectError || !subjectData) {
        const errMsg = subjectError ? subjectError.message : `Subject "english-literature" not found for units.`;
        setError(new Error(errMsg));
    ...
    <>
      <main className="min-h-screen bg-white flex flex-col items-center justify-start py-10 m-10">
        <div className="w-full max-w-5xl px-4">
          ...
          >
            IAL <span className="bg-[#1A69FA] px-2 py-1 -rotate-1 inline-block"><span className="text-[#FFFFFF]">English Literature</span></span> Past Papers
          </h1>
          ...
          >
            <span className="text-md font-medium text-black tracking-tight">
              <span className="font-[501]">Exam code:</span> WET0/XET01/YET01
            </span>
          </div>
          ...
          <h3
            ...
            style={{ fontFamily: "Poppins, sans-serif" }}
          >
            Explore our collection of Edexcel IAL English Literature Past Papers and Mark Schemes below. Practicing with IAL English Literature past papers is one of the most effective ways to pinpoint the topics that need more focus—helping you revise smarter and prepare confidently for your upcoming exam
          </h3>

          <Link href={`/subjects/${subjectSlug}/IAL/resources`}>

...
export default function IGCSECommunityNotesPage() {
  ...
  // For example, if your file is `pages/subjects/[slug]/IGCSE/communityNotes.js`:
  ...
  const subjectName = 'Information Technology'; // <<< CHANGE THIS TO BE DYNAMIC BASED ON YOUR ROUTING
    ...
    async function fetchExamCode() {
      try {
        ...
        if (data && data.code) {
          ...
          setExamCode('N/A');
          console.warn(`Subject "information-technology" not found or has no associated exam code.`);
        }
      } catch (err) {
  ...
  useEffect(() => {
    ...
    const fetchUnits = async () => {
      ...
      if (subjectError || !subjectData) {
        setError(subjectError || new Error('Subject "Information Technology" not found.'));
        return;
      }
  ...
  useEffect(() => {
    ...
    async function fetchNotes() {
      setLoading(true);
      setError(null);
      // Get subject id for Information Technology IGCSE
      const { data: subjectData, error: subjectError } = await supabase
        .from('subjects')
        .select('id')
        .eq('name', subjectName)
      ...
      <main className="min-h-screen bg-white flex flex-col items-center justify-start py-10 m-10">
        <div className="w-full max-w-5xl px-4">
          ...
          <div className="blur-sm pointer-events-none select-none">
            ...
            >
              IGCSE <span className="bg-[#1A69FA] px-2 py-1 -rotate-1 inline-block"><span className="text-[#FFFFFF]">Information Technology</span></span> Eduvance Notes
            </h1>
            ...
            <h3
              ...
              style={{ fontFamily: "Poppins, sans-serif" }}
            >
              Explore our collection of Edexcel A Level Information Technology community-contributed resources, including detailed notes, explanations, and revision tips. These resources are perfect for deepening your understanding, clarifying tricky concepts, and supporting your study alongside past papers.
            </h3>

...
export default function IALCommunityNotesPage() {
  ...
  useEffect(() => {
    ...
    async function fetchNotes() {
      setLoading(true);
      setError(null);
      // Get subject id for Mathematics IAL
      const { data: subjectData, error: subjectError } = await supabase
        .from('subjects')
        .select('id')
        .eq('name', subjectName)
      ...
      <main className="min-h-screen bg-white flex flex-col items-center justify-start py-10 m-10">
        <div className="w-full max-w-5xl px-4">
          ...
          <div className="blur-sm pointer-events-none select-none">
            ...
            >
              IAL <span className="bg-[#1A69FA] px-2 py-1 -rotate-1 inline-block"><span className="text-[#FFFFFF]">Mathematics</span></span> Eduvance Notes
            </h1>

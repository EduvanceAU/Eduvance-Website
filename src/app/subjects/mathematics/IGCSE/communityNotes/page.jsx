  ...
  useEffect(() => {
    ...
    const fetchUnits = async () => {
      ...
      if (subjectError || !subjectData) {
        setError(subjectError || new Error(`Subject "mathematics" not found for units.`));
        return;
      }
  ...
  useEffect(() => {
    ...
    async function fetchNotes() {
      ...
      const { data: subjectData, error: subjectError } = await supabase
        ...
        .single();
      if (subjectError || !subjectData) {
        setError(subjectError || new Error(`Subject "mathematics" not found for notes.`));
        setLoading(false);
        return;
      }
          ...
          <div className="blur-sm pointer-events-none select-none">
            ...
            >
              <span className="text-md font-medium text-black tracking-tight">
                <span className="font-[501]">Exam code:</span> 4MB1, 4PM1 {/* This now uses the state */}
              </span>
            </div>

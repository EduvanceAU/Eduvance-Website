...
const SubjectButtons = () => {
  ...
  useEffect(() => {
    async function fetchSubjects() {
      try {
        const res = await fetch('/api/mysql/subjects?type=IAL', { cache: 'no-store' });
...
export default function IALPastPapersPage() {
  ...
  const fetchPapers = async () => {
    ...
    try {
      // Fetch subject and code via MySQL API
      const subjRes = await fetch(`/api/mysql/subject?name=${encodeURIComponent(subjectName)}&type=IAL`, { cache: 'no-store' });
      const subjJson = await subjRes.json();
      if (!subjRes.ok) throw new Error(subjJson.error || 'Subject not found');
      setExamCode(subjJson.subject?.code || 'N/A');

      const res = await fetch(`/api/mysql/papers?subject=${encodeURIComponent(subjectName)}&type=IAL`, { cache: 'no-store' });
      const json = await res.json();

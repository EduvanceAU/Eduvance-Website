...
// SubjectButtons component remains largely the same, but its links are now '/subjects/${slug}/IGCSE/resources'
  ...
  return (
    <div className="flex flex-wrap gap-2 mb-6">
      {subjects.map((name, index) => {
        ...
        return (
          <Link key={index} href={`/subjects/${slug}/IGCSE/resources`}> {/* Ensure this href is correct for your routing */}
    ...
    const fetchResources = async () => {
      try {
        ...
        if (subjectError || !subjectData) {
          setError(subjectError || new Error(`Subject "${subjectName}" not found for resource fetching.`));
          setLoading(false);
          return;
        }
      ...
      <main className="min-h-screen bg-white flex flex-col items-center justify-start py-10 m-10">
        <div className="w-full max-w-5xl px-4">
          <h1 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-[#000000] mb-8 text-left tracking-[-0.035em]" style={{ fontFamily: "Poppins, sans-serif" }}>
            IGCSE <span className="bg-[#1A69FA] px-2 py-1 -rotate-1 inline-block"><span className="text-[#FFFFFF]">{subjectName}</span></span> Resources
          </h1>
          ...
          <h3 className="text-sm sm:text-md lg:text-lg font-[500] leading-6 text-[#707070] mb-8 text-left max-w-4xl tracking-[-0.015em]" style={{ fontFamily: "Poppins, sans-serif" }}>
            Access a wide range of Edexcel IGCSE {subjectName} resources—all in one place. Whether you're brushing up on concepts or aiming to master exam strategies, these materials are designed to support your revision and boost your performance
          </h3>

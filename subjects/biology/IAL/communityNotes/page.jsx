  ...
  useEffect(() => {
    ...
    const fetchUnits = async () => {
      ...
      if (subjectError || !subjectData) {
        setError(subjectError || new Error(`Subject "biology" not found for units.`));
        return;
      }
  ...
  useEffect(() => {
    ...
    async function fetchNotes() {
      ...
      const { data: subjectData, error: subjectError } = await supabase
        ...
        .single();
      if (subjectError || !subjectData) {
        setError(subjectError || new Error(`Subject "biology" not found for notes.`));
        setLoading(false);
        return;
      }
      ...
      <main className="min-h-screen bg-white flex flex-col items-center justify-start py-10 m-10">
        <div className="w-full max-w-5xl px-4">
          ...
          <div className="blur-sm pointer-events-none select-none">
            ...
            >
              IAL <span className="bg-[#1A69FA] px-2 py-1 -rotate-1 inline-block"><span className="text-[#FFFFFF]">Biology</span></span> Eduvance Notes
            </h1>
            ...
            >
              <span className="text-md font-medium text-black tracking-tight">
                <span className="font-[501]">Exam code:</span> WBI1/XBI11/YBI11 {/* This now uses the state */}
              </span>
            </div>
